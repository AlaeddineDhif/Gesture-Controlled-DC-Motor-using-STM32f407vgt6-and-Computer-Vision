...
/* USER CODE BEGIN PV */
uint8_t uart_rx_buffer[32];  // Buffer pour stocker les données UART
volatile uint8_t uart_rx_flag = 0;  // Drapeau pour indiquer que des données sont reçues
uint8_t motor_speed = 0;   // Vitesse du moteur (0-100%)
uint8_t motor_direction = 0;  // Direction du moteur (0: arrêt, 1: anti-horaire, 2: horaire)
char debug_buffer[DEBUG_MSG_SIZE];  // Buffer pour les messages de debug
/* USER CODE END PV */
